...
encrypt_response = function(assertion, options, cb) {

  var options_encrypt = {
    rsa_pub: options.rsa_pub,
    pem: options.certificate,
...
validate_and_decrypt = function(saml_response, apc_private_keys, idp_certificates, allow_unencrypted, ignore_signature, require_session_index, ignore_timing, notbefore_skew, audience_allowed, cb) {
  var user;
  ...
  return async.waterfall([
    function(cb_wf) {
      ...
      for (i = 0; i < ref1.length; i++) {
        cert = ref1[i]
        try {
          signed_data = check_saml_signature_with_private(saml_response_str, cert);
        } catch (error) {
          ex = error;
          return cb_wf(new Error("SAML Assertion signature check failed! (Certificate \#" + (i + 1) + " may be invalid. " + ex.message));
        }
        ...
        return cb_wf(null, saml_response)
      /*}
      return cb_wf(new Error("SAML Assertion signature check failed! (checked " + ref1.length + " certificate(s))"));*/
    }, function(encrypted_assertion, cb_wf) {
...
module.exports.APConnector = APConnector = (function() {
  ...
  APConnector.prototype._assert = function(identity_provider, options, cb) {
    ...
    return async.waterfall([
      function(cb_wf) {
        ...
        return function(response_buffer, cb_wf) {
          ...
          switch (false) {
            case saml_response.getElementsByTagNameNS(XMLNS.SAMLP, 'Response').length !== 1:
              ...
              return validate_and_decrypt(saml_response, [options.private_key], options.certificate, options.allow_unencrypted_assertion, options.ignore_signature, options.require_session_index, options.ignore_timing, options.notbefore_skew, options.audience, cb_wf);
            case saml_response.getElementsByTagNameNS(XMLNS.SAMLP, 'LogoutResponse').length !== 1:
              if (!check_status_success(saml_response)) {
                return cb_wf(new SAMLError("SAML Response was not success!", {
                  status: get_status(saml_response)
        ...
        return cb_wf(null, { decrypted: decrypted_assertion, saml_response: saml_response})
      }
    ], cb);
  };
  ...
  APConnector.prototype._encrypt_response = function(identity_provider, options, cb) {
    ...
    return async.waterfall([
      function(cb_wf) {
        if (options.new_attributes) {
          var dom_decrypted_assertion = decrypted_assertion;
          var assertion_element = dom_decrypted_assertion.getElementsByTagNameNS(XMLNS.SAML, 'Assertion')[0];
          ...
          var new_attribute_statement = new xmldom.XMLSerializer().serializeToString(dom_decrypted_assertion);
          new_attribute_statement = new_attribute_statement.replace(' xmlns:saml2=""', '');
          return encrypt_response(new_attribute_statement, options, cb_wf)
        } else {
          decrypted_assertion = new xmldom.XMLSerializer().serializeToString(decrypted_assertion);
          return encrypt_response(decrypted_assertion, options, cb_wf)
        }
      }, function(encrypt_data, cb_wf) {
        var dom_encrypted_data = (new xmldom.DOMParser()).parseFromString(encrypt_data);
        ...
        var signed = sign_response_request(unsigend_response, options.private_key, options, options.certificate);
