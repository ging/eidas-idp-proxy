...
encrypt_response = function(assertion, options, cb) {
  ...
  var options_encrypt = {
    ...
    encryptionAlgorithm: 'http://www.w3.org/2001/04/xmlenc#aes256-gcm',
  ...
  async.waterfall([
    ...
    function encrypt_content(symmetricKey, cb) {
      switch (options.encryptionAlgorithm) {
        case 'http://www.w3.org/2001/04/xmlenc#aes256-gcm':
    ...
    function encrypt_key(symmetricKey, encryptedContent, cb) {
      ...
      xmlenc.encryptKeyInfo(symmetricKey, options, function(err, keyInfo) {
        if (err) {
          return cb(err);
        }
...
function encryptWithAlgorithm(algorithm, symmetricKey, ivLength, content, encoding, callback) {
  ...
  crypto.randomBytes(ivLength, function(err, iv) {
    ...
    return callback(null, Buffer.concat([nonce, new Buffer(encrypted, 'binary'), tag])/*.toString('hex')*/);
  });
}
...
validate_and_decrypt = function(saml_response, sp_private_keys, idp_certificates, allow_unencrypted, ignore_signature, require_session_index, ignore_timing, notbefore_skew, audience_allowed, cb) {
  var user;
  ...
  APConnector.prototype._assert = function(identity_provider, options, cb) {
    ...
    return async.waterfall([
      function(cb_wf) {
        ...
        return function(response_buffer, cb_wf) {
          ...
          switch (false) {
            case saml_response.getElementsByTagNameNS(XMLNS.SAMLP, 'Response').length !== 1:
              ...
              response.type = 'authn_response';
              return validate_and_decrypt(saml_response, [_this.private_key].concat(_this.alt_private_keys), identity_provider.certificates, options.allow_unencrypted_assertion, options.ignore_signature, options.require_session_index, options.ignore_timing, options.notbefore_skew, options.audience, cb_wf);
            case saml_response.getElementsByTagNameNS(XMLNS.SAMLP, 'LogoutResponse').length !== 1:
              if (!check_status_success(saml_response)) {
                return cb_wf(new SAMLError("SAML Response was not success!", {
                  status: get_status(saml_response)
        ...
        return encrypt_response(result.decrypted, options, cb_wf)
        /*_.extend(response, result);
        return cb_wf(null, response);*/
      }
    ], cb);
